...
model <- make_model("X -> M -> Y") %>%
         ...
         set_parameters(node = "X",  parameters = c(.2, .8, .8, .2)) %>% 
         set_parameters(node = "M", parameters = c(.2, 0, .8, 0)) %>%
...
CausalQueries:::plot_dag(frontdoor)
...
```{r, echo = FALSE}
selection <- make_model("X <- W1 -> W2 -> X -> Y <- Z -> W2; W1 -> S")
CausalQueries:::plot_dag(selection)
```
...
par(mfrow = 1:2)
CausalQueries:::plot_dag(model_1)
# title(main = list("X not known to be randomized", cex = .5))
CausalQueries:::plot_dag(model_2)
