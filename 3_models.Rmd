...
list('Posterior on ATE' = data.frame(p = as.vector(ATE_dist)),
     'Posterior on PC' = data.frame(p = as.vector(PC_dist))) |>
  bind_rows(.id = "Q") |>
...
df <- cbind(
  df |> select(starts_with("ate")) |>
    pivot_longer(cols=ate.1:ate.12000, 
                 names_to = "ID", values_to = "ate"),
  df |> select(starts_with("confound")) |>
    pivot_longer(cols=confound.1:confound.12000, 
                 names_to = "ID1", values_to = "confound")) |>
  select(ate, confound)
