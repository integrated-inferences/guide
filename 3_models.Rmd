...
if(run){
  write_rds(update_model(model, data, refresh = 0), "saved/appendix_XY_model_1.rds")
  }
...
list('Posterior on ATE' = data.frame(p = ATE_dist[[1]]),
     'Posterior on PC' = data.frame(p = PC_dist[[1]])) |>
  bind_rows(.id = "Q") |>
  ggplot(aes(p)) + geom_histogram() + facet_grid(~Q) + xlim(c(0,1)) + theme_bw()
...
```{r, message = FALSE, warning = FALSE, include = FALSE}
if(run){
  write_rds(update_model(model, data5000, refresh = 0, iter = 6000), "saved/appendix_XY_model_5000.rds")
    write_rds(update_model(model, data100, refresh = 0, iter = 6000), "saved/appendix_XY_model_100.rds")
...
if(run){
  ...
  data_sparse <- select(data, X, Y)
  
  updated  <- update_model(model, data_sparse, refresh = 0, iter = 6000)
  
  write_rds(updated, "saved/appendix_modelled_confound_sparse.rds")


  query_distribution(updated, 
                     list(ate = "c(Y[X=1] - Y[X=0])",
                          confound = "c(Y[C=1] - Y[C=0])"),
                     using = "posteriors") |>


write_rds("saved/appendix_modelled_confound_confound.rds")
}
...
plot(df$confound, df$ate, ylab = "ATE", xlab = "Confound", cex = .2)
```
...
```{r, message = FALSE, warning = FALSE, include = FALSE}
if(run){
  write_rds(update_model(model, data, refresh = 0), "saved/appendix_XMY_simple.rds")
  }
...
```{r, message = FALSE, warning = FALSE, include = FALSE}
if(run){
  write_rds(update_model(model, data), "saved/appendix_mod_simple.rds")
  }
...
```{r, echo = FALSE}
kable(app_PC_NoC, caption = "Probability $X$ caused $Y$ from model updated without data on $C$")
```

```{r, echo = FALSE}
kable(app_PC_C, caption ="Probability $X$ caused $Y$ from model updated using data on $C$")
...
kable(actual_cause_2, caption = "Inferences when Suzy does *not* throw")
```
...
par(mfrow = c(2,2))
hist(back$dist4[[1]], xlim = c(-1, 1)*.4, main = "Small n, backdoor open", xlab = "ATE")
abline(v = v, col = "red")
hist(back$dist3[[1]], xlim = c(-1, 1)*.4, main = "Small n, backdoor blocked", xlab = "ATE")
abline(v = v, col = "red")
hist(back$dist2[[1]], xlim = c(-1, 1)*.4, main = "Large n, backdoor open", xlab = "ATE")
abline(v = v, col = "red")
hist(back$dist1[[1]], xlim = c(-1, 1)*.4, main = "Large n, backdoor blocked", xlab = "ATE")
abline(v = v, col = "red")
...
par(mfrow = c(1,2))
hist(frontdoor$Q1[[1]], xlim = c(-.3, .3), main= "Inferences given X,Y, data only", xlab = "ATE")
hist(frontdoor$Q2[[1]], xlim = c(-.3, .3), main = "Inferences given X,Y, and M data", xlab = "ATE")
```
...
Consider the following model from @bareinboim2016causal (their Figure 4C). The key feature is that data is only seen for units with $S=1$ ($S$ for sampling).
...
selection <- make_model("X <- W1 -> W2 -> X -> Y <- Z -> W2; W1 -> S") |>
  ...
  set_restrictions(labels = list(W2 = c("0001", "1111")), keep = TRUE) |>
    set_parameters(statement = "(S[W1=1] > S[W1=0])", parameters = .14) |>
    set_parameters(statement = "(Y[X=1] != Y[X=0])", parameters = 0) |>
    set_parameters(node = "X", nodal_type = "0001", parameters = .85)
...
```{r frank4, echo = FALSE}
kable(frank[[4]][,-c(1,3)], 
      caption = "Clue is informative after combining studies linking $K$ to $Z$ and $Z$ to $Y$")
```
